...
export default function Portfolio({ user }) {
  ...
  const ensurePortfolio = async () => {
    ...
    const createRes = await fetch('/api/portfolios', {
      ...
      body: JSON.stringify({ user, name: `${user}'s Portfolio` }),
    });
  ...
  const load = async () => {
    ...
    const [hRes, tRes] = await Promise.all([
      fetch(`/api/portfolios/${pid}/holdings`),
      fetch(`/api/portfolios/${pid}/trades`),
    ]);
  ...
  const submit = async (e) => {
    ...
    await fetch(`/api/portfolios/${portfolioId}/trades`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...form,
    ...
    <div>
      ...
      <div className="card">
        ...
        {loading ? <p className="loading">Loading...</p> : holdings.length === 0 ? (
          <p className="loading">No holdings yet. Add a trade below.</p>
        ) : (
          <table>
            <thead>
              <tr>
